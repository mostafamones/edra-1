...
export function StudentImportDialog({
  ...
  const handleFileSelect = useCallback((selectedFile: File | null) => {
    ...
    if (!isCsv && !isJson) { toast.error("Please select a CSV or JSON file"); return }
  ...
  const handleDownloadTemplate = useCallback(() => {
    ...
    else downloadJSONTemplate(templateOptions)
  }, [format, levels, groups, customFields, availableSchedules])
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      const { processed, duplicates } = processStudentsForImport(parsedStudents, {
        ...
        defaultBranchId: defaultGroupId ? parseInt(defaultGroupId) : undefined,
      })
      ...
      const progressInterval = setInterval(() => {
        setImportProgress((prev) => { if (prev >= 90) { clearInterval(progressInterval); return 90 } return prev + 10 })
      }, 100)
      ...
      const response = await fetch("/api/students/bulk", {
        ...
        body: JSON.stringify({
          ...
          academyId,
          defaultLevelId: defaultLevelId ? parseInt(defaultLevelId) : undefined,
          defaultGroupId: defaultGroupId ? parseInt(defaultGroupId) : undefined,
      ...
      if (totalErrors === 0) toast.success(`Successfully imported ${result.created} student${result.created > 1 ? "s" : ""}`)
  ...
  const getPreviewColumns = () => {
    const cols = ["Name", "Level", "Group", "Schedule", "Email"]
    customFields.forEach((f) => cols.push(toSnakeCase(f.name)))
    return cols
  }
      ...
      <tr key={student.full_name + "-" + index} className={hasErrors ? "bg-destructive/10" : ""}>
        ...
        <td className="p-2 border-b">
          {hasErrors ? (
            ...
            <Badge variant="outline" className="text-xs text-green-600 border-green-600">Valid</Badge>
          )}
        </td>
      </tr>
    )
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent
        className={`max-h-[90vh] overflow-hidden flex flex-col transition-all duration-300 ${
          step === "preview" ? "max-w-[90vw] lg:max-w-[1200px]" : "max-w-[1000px] w-[1000px]"
        }`}
      >
        <DialogHeader>
          ...
          <div className="space-y-4">
            <Tabs value={format} onValueChange={(v) => setFormat(v as ImportFormat)}>
              <TabsList className="grid w-full grid-cols-2">
                <TabsTrigger value="csv"><IconFileSpreadsheet className="size-4 mr-2" />CSV</TabsTrigger>
                <TabsTrigger value="json"><IconCode className="size-4 mr-2" />JSON</TabsTrigger>
              </TabsList>
              ...
              <TabsContent value={format} className="mt-4 space-y-4">
                <div className="flex items-center justify-between p-3 rounded-lg border bg-muted/30">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Get a template file with the correct format</p>
                  </div>
                  <Button variant="outline" size="sm" onClick={handleDownloadTemplate}>
                    <IconDownload className="size-4 mr-2" />Download {format.toUpperCase()}
                  </Button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-2">
                    ...
                    <Select value={defaultLevelId || "__none__"} onValueChange={(v) => { setDefaultLevelId(v === "__none__" ? "" : v); setDefaultGroupId("") }}>
                      <SelectTrigger id="default-level" className="w-full"><SelectValue placeholder="Optional" /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="__none__">None</SelectItem>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setDefaultGroupId(v === "__none__" ? "" : v)}
                      disabled={!defaultLevelId || availableGroups.length === 0}
                    >
                      <SelectTrigger id="default-group" className="w-full"><SelectValue placeholder="Optional" /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="__none__">None</SelectItem>
                        {availableGroups.map((g) => <SelectItem key={g.id} value={g.id.toString()}>{g.name}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="flex flex-col items-center gap-2">
                    {format === "csv" ? <IconFileSpreadsheet className="size-8 text-muted-foreground" /> : <IconCode className="size-8 text-muted-foreground" />}
                    <p className="text-sm font-medium">Drag and drop your {format.toUpperCase()} file here</p>
                    <p className="text-xs text-muted-foreground">or click to browse</p>
                  </div>
                </div>

                <div className="text-xs text-muted-foreground space-y-1">
                  <p><strong>Required columns:</strong> full_name, level</p>
                  <p><strong>Optional columns:</strong> group, schedule, email</p>
                  {customFields.length > 0 && (
                    <p><strong>Custom fields:</strong> {customFields.map((f) => toSnakeCase(f.name)).join(", ")}</p>
                  )}
                </div>
              </TabsContent>
            </Tabs>
          ...
          <div className="space-y-4 w-full flex-1 overflow-hidden flex flex-col">
            <div className="flex items-center justify-between p-3 rounded-lg bg-muted/30">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-2xl font-bold text-green-600">{parsedStudents.filter((s) => !s.errors || s.errors.length === 0).length}</p>
                  <p className="text-xs text-muted-foreground">Valid</p>
                </div>
                <div className="w-px h-8 bg-border" />
                <div>
                  <p className="text-2xl font-bold text-destructive">{parsedStudents.filter((s) => s.errors && s.errors.length > 0).length}</p>
            ...
            {parsedStudents.some((s) => s.errors && s.errors.length > 0) && (
              <div className="p-3 rounded-lg bg-destructive/10 border border-destructive/20">
                <p className="text-sm font-medium text-destructive mb-2">Errors that will be skipped:</p>
                <ul className="text-xs text-muted-foreground space-y-1">
                  {parsedStudents.filter((s) => s.errors && s.errors.length > 0).slice(0, 5).map((student, i) => (
                    <li key={i}>{student.full_name}: {student.errors?.map((e) => e.message).join(", ")}</li>
                  ))}
                  {parsedStudents.filter((s) => s.errors && s.errors.length > 0).length > 5 && (
                    <li className="italic">...and {parsedStudents.filter((s) => s.errors && s.errors.length > 0).length - 5} more</li>
                  )}
                </ul>
              </div>
            )}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-3 gap-4">
              <div className="p-4 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
                ...
                <p className="text-3xl font-bold text-green-700 dark:text-green-500 mt-1">{importResults.created}</p>
              </div>
              <div className="p-4 rounded-lg bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800">
                ...
                <p className="text-3xl font-bold text-yellow-700 dark:text-yellow-500 mt-1">{importResults.skipped}</p>
              </div>
              <div className="p-4 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
                ...
                <p className="text-3xl font-bold text-red-700 dark:text-red-500 mt-1">{importResults.errors.length}</p>
              </div>
            </div>
              ...
              <div className="p-3 rounded-lg bg-muted/30">
                ...
                <ScrollArea className="h-20">
                  <ul className="text-xs text-muted-foreground space-y-0.5">
                    {importResults.skippedNames.map((name, i) => <li key={i}>• {name}</li>)}
                  </ul>
                </ScrollArea>
              </div>
            )}
              ...
              <div className="p-3 rounded-lg bg-destructive/10 border border-destructive/20">
                ...
                <ScrollArea className="h-20">
                  <ul className="text-xs text-destructive space-y-0.5">
                    {importResults.errors.map((error, i) => <li key={i}>• {error}</li>)}
                  </ul>
                </ScrollArea>
              </div>
            )}
